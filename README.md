...
- **Non-English indexer / metadata support** — Let monitored authors and searches pull from language-tagged catalogues and filter results by language. Concretely: expose a configurable language list per monitored author (default: all), propagate it to indexer queries (Prowlarr's `Categories` + language filters, Jackett `/api?cat=7000&...`), and surface the language tag in search results and the wanted-books view. Hardcover and Google Books metadata already carry a `language` field that's currently ignored — start by persisting it. Relevant to French/Dutch/German users whose libraries are mixed-language and where indexer results in the "wrong" language are currently indistinguishable.
